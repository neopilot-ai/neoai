...
#    darwin: https://github.com/neopilot-ai/neoai/releases/download/${LATEST_MS_TAG}/NeoAI-darwin-${LATEST_MS_TAG}.zip
...
URL_BASE=https://github.com/neopilot-ai/neoai/releases/download/${LATEST_MS_TAG}

# to make testing on forks easier
if [[ "$CI_WINDOWS" == "True" ]]; then
  # BUILD_REPOSITORY_URI = e.g. https://github.com/neopilot-ai/neoai
...
# e.g. generateJson NeoAI-darwin-1.33.0.zip
generateJson() {
  local assetName=$1
...
cd versions
git config user.email "neoai-ci@not-real.com"
git config user.name "NeoAI CI"
...
if [[ "$OS_NAME" == "osx" ]]; then
  ...
  ASSET_NAME=NeoAI-darwin-${LATEST_MS_TAG}.zip
  VERSION_PATH="darwin"
  JSON="$(generateJson ${ASSET_NAME})"
  updateLatestVersion "$VERSION_PATH" "$JSON"
elif [[ "$CI_WINDOWS" == "True" ]]; then
  # system installer
  ASSET_NAME=NeoAISetup-${BUILDARCH}-${LATEST_MS_TAG}.exe
  VERSION_PATH="win32/${BUILDARCH}/system"
  JSON="$(generateJson ${ASSET_NAME})"
  updateLatestVersion "$VERSION_PATH" "$JSON"

  # user installer
  ASSET_NAME=NeoAIUserSetup-${BUILDARCH}-${LATEST_MS_TAG}.exe
  VERSION_PATH="win32/${BUILDARCH}/user"
  JSON="$(generateJson ${ASSET_NAME})"
  updateLatestVersion "$VERSION_PATH" "$JSON"

  # windows archive
  ASSET_NAME=NeoAI-win32-${BUILDARCH}-${LATEST_MS_TAG}.zip
  ...
  # as examples
  ASSET_NAME=NeoAI-linux-${VSCODE_ARCH}-${LATEST_MS_TAG}.tar.gz
