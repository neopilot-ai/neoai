...
export function CopyInstall() {
  ...
  const copyCommand = () => {
    navigator.clipboard.writeText("npx trans@latest");
    setCopied(true);
  ...
  return (
    <div className="flex mt-4 border border-dashed border-muted-foreground p-2 px-4 text-sm w-full relative">
      ...
      >
        ...
        <span className="text-primary truncate">$ npx trans@latest</span>
      </button>

      <div className="flex items-center space-x-2 ml-auto">
        <Link href="https://npmjs.com/package/trans">
          <NPMIcon />
        </Link>
        <Link href="https://github.com/neopilot-ai/trans">
          <GitHubIcon />
        </Link>
      </div>
