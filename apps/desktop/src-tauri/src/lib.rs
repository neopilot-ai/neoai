...
async fn create_preloaded_search_window(
    ...
    .resizable(false)
    .user_agent("Mozilla/5.0 (compatible; Neoai Desktop App)")
    .transparent(true)
    .decorations(false)
...
fn get_app_url() -> String {
    ...
    let env = env::var("NEOAI_ENV")
        .unwrap_or_else(|_| {
            option_env!("NEOAI_ENV")
    ...
    match env.as_str() {
        ...
        "staging" => {
            let url = "https://beta.neoai.khulnasoft.com".to_string();
            println!("🌍 Using staging URL: {}", url);
            url
        },
        "production" | "prod" => {
            let url = "https://app.neoai.khulnasoft.com".to_string();
...
fn handle_deep_link_event(app_handle: &tauri::AppHandle, urls: Vec<String>) {
    for url in &urls {
        if url.starts_with("neoai://") {
            // Extract the path from the deep link
            let path = url.strip_prefix("neoai://").unwrap_or("");
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Set the default app menu to restore the Neoai menu
