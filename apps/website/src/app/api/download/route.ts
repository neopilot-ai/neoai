...
export async function GET(request: NextRequest) {
  try {
    ...
    const releaseResponse = await fetch(
      "https://api.github.com/repos/neoai-ai/neoai/releases/latest",
      {
        headers: {
          "User-Agent": "Neoai-Desktop-Downloader",
    ...
    // Extract version number from tag (remove 'neoai-v' prefix)
    const version = tag_name.replace(/^neoai-v?/, "");

    // Construct DMG filename based on version and platform
    const filename = `Neoai_${version}_${platform}.dmg`;

    // Construct download URL using the full tag name
    const downloadUrl = `https://github.com/neoai-ai/neoai/releases/download/${tag_name}/${filename}`;
