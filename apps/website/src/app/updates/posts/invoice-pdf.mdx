...
summary: "Discover how we're implementing PDF invoice generation in Neoai, using React-based solutions for customizable, efficient document creation."
...
<br />
We are building a new experience for invoices in Neoai. You'll be able to create and send invoices to your customers, as well as generate PDFs for each invoice. 
...
The invoice is then generated and saved to your [Vault](https://neoai.khulnasoft.com/vault), so we can match it to incoming transactions and mark it as paid. 
...
```tsx
import { InvoiceTemplate, renderToStream } from "@neoai/invoice";
import { getInvoiceQuery } from "@neoai/supabase/queries";
...
You can find the full code for the invoice template [here](https://go.neoai.khulnasoft.com/inv).
