...
share how we solved it in Neoai using NextJS and server-actions.

<br />
Because we have a monorepo we started with creating a new package called `@neoai/events`
where we install `@logsnag/next`.

<br />
The package includes all the events we want to track in Neoai for example:
...
When you sign in to Neoai we first ask you about tracking, we want you to keep your privacy. This is done by showing a `Toast` component with the option to Accept or Decline, we save this decision
...
We have a lot of events and charts pushing us to take right decisions, you can find the source code for this in our repository [here](https://git.new/neoai).
