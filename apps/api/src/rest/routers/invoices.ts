...
import {
  ...
  isInvoiceNumberUsed,
  updateInvoice,
} from "@neoai/db/queries";
import { calculateTotal } from "@neoai/invoice/calculate";
import { transformCustomerToContent } from "@neoai/invoice/utils";
import type { GenerateInvoicePayload } from "@neoai/jobs/schema";
      ...
      data: result.data.map((invoice) => {
        ...
        return {
          ...
          bottomBlock: invoice.bottomBlock
            ...
            : null,
          pdfUrl: token
            ? `${process.env.NEOAI_DASHBOARD_URL}/api/download/invoice?token=${token}`
            : null,
          previewUrl: token
            ? `${process.env.NEOAI_DASHBOARD_URL}/i/${token}`
            : null,
        };
      }),
    };
