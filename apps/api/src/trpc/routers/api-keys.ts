...
import { apiKeyCache } from "@neoai/cache/api-key-cache";
import {
  deleteApiKey,
  getApiKeysByTeam,
  upsertApiKey,
} from "@neoai/db/queries";
import { ApiKeyCreatedEmail } from "@neoai/email/emails/api-key-created";
    ...
    .mutation(async ({ ctx: { db, teamId, session, geo }, input }) => {
      ...
      if (data) {
        try {
          ...
          resend.emails.send({
            from: "Neoaibot <neoaibot@khulnasoft.com>",
