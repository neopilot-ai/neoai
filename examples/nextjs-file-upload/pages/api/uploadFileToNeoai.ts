...
import { NeoaiClient } from "@neoai/sdk";
import { NextFunction } from "express";
import { IncomingMessage, ServerResponse } from "http";

const neoaiClient = new NeoaiClient({ apiKey: process.env.NEOAI_API_KEY });
...
export default async function (req: NextApiRequest, res: NextApiResponse): Promise<void> {
  ...
  try {
    const url = await uploadFileToNeoai(multerFile);
...
/** Uploads a file to Neoai, returning the uploaded URL. */
async function uploadFileToNeoai(file: Express.Multer.File): Promise<string> {
  const uploadPayload = await neoaiClient.fileUpload(file.mimetype, file.originalname, file.size);
  ...
  try {
    ...
    throw new Error("Failed to upload file to Neoai", { cause: e });
  }
}
