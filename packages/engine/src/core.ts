...
export class APIPromise<T> extends Promise<T> {
   ...
   * or add one of these imports before your first `import … from '@neoai/engine'`:
   * - `import '@neoai/engine/shims/node'` (if you're running on Node)
   * - `import '@neoai/engine/shims/web'` (otherwise)
   */
...
const validatePositiveInteger = (name: string, n: unknown): number => {
  if (typeof n !== "number" || !Number.isInteger(n)) {
    throw new NeoaiError(`${name} must be an integer`);
  }
  if (n < 0) {
    throw new NeoaiError(`${name} must be a positive integer`);
  }
  return n;
};
...
export function debug(action: string, ...args: any[]) {
  if (typeof process !== "undefined" && process?.env?.["DEBUG"] === "true") {
    console.log(`Neoai:DEBUG:${action}`, ...args);
  }
}
