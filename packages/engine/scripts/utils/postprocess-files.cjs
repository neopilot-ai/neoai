...
const pkgImportPath = process.env["PKG_IMPORT_PATH"] ?? "@neoai/engine/";
...
function replaceRanges(code, replacer) {
  const replacements = [];
  replacer({
    replace: (range, replacement) => replacements.push({ range, replacement }),
  });
  ...
  for await (const file of walk(path.resolve(__dirname, "..", "..", "dist"))) {
    ...
    if (file.startsWith(distSrcDir) && !file.endsWith("_shims/index.d.ts")) {
      ...
      transformed = transformed.replace(
        new RegExp(
          "unknown extends (typeof )?\\S+ \\? \\S+ :\\s*".replace(
            /\s+/,
            "\\s+",
          ),
          "gm",
    ...
    if (file.endsWith(".d.ts")) {
      ...
      // if we have `import { type Readable } from '@neoai/engine/_shims/index'`,
      // tsc sometimes replaces `Readable` with `import("stream").Readable` inline
      // in the output .d.ts
      transformed = transformed.replace(
        /import\("stream"\).Readable/g,
        "Readable",
      );
    }
